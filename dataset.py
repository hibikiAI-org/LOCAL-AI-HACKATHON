...
parser.add_argument('split_no')

args = parser.parse_args()
split = int(args.split_no)


print("Loading dataset")

start_idx = split*split_size
end_idx = min(11096414, (split+1)*split_size)

#print(start_idx, end_idx)
os.makedirs(f"./audios/{split:05d}", exist_ok=True)

train_ds = load_dataset("reazon-research/reazonspeech", "all", cache_dir="/mnt/data", split=f'train[{start_idx}:{end_idx}]')
...
for i in range(len(train_ds)):
  try:
    val = train_ds[i]
    src_path = val['audio']['path'] 
    duration = sf.info(src_path).duration
    ...
    dst_path = f'{split:05d}/{i:05d}.flac'
    os.symlink(src_path, './audios/'+dst_path)

    
    phoneme_lst, kana = phonemizer(text)
    kana_set = set(list(kana))
    phonemes = "|".join(phoneme_lst)
    if not (kana_set <= acceptable_symbols):
      #print(text)
      #print(kana)
      continue

    speed = len(re.sub(r"([|]|、|。|！|？|\!|\?)", "", kana))/duration
    if speed > 15 or speed < 7.5:
      #if speed > 15:
      #  print(speed, text)
      #  shutil.copy(src_path, "./output/tmp/")
      continue
    ...
    cnt += 1
    print(f"Split-{split}: {cnt}/{i}")


  except Exception as e: 
    print(e)

with open(f"json/{split}.json", 'w') as f_o:
  json.dump(records, f_o, ensure_ascii=False, indent=2)
